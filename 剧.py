...
class Spider(Spider):
    ...
    # ====================== 弹幕核心接口（新增）======================
    def getDanmu(self, flag, id, vipFlags):
        try:
            play = self.playerContent(flag, id, vipFlags)
            real_url = play.get("url", "")
            if not real_url:
                return {"danmu": "", "header": {}}
            dm_id = self.md5(real_url)
            # 公共弹幕接口
            danmu_url = f"https://api.dm520.com/api/{dm_id}"
            return {
                "danmu": danmu_url,
